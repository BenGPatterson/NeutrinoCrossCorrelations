...
def _make_fake_data():
    ...
    if not os.path.exists(FakeEventDir):
        os.mkdir(FakeEventDir)
    if not os.path.exists(FakeAeffDir):
        os.mkdir(FakeAeffDir)
    ...
    for i, season in enumerate(seasons):
        ...
        logEcol = ([2.01]*len(ra) + [3.01]*len(ra) +
                   [4.01]*len(ra) + [5.01]*len(ra))
        racol = list(ra)*4
        deccol = list(dec)*4
        ...
        if i <= 4:
            ...
            a = Table({'log10(E_nu/GeV)_min': Emin,
                       ...
                       'A_Eff[cm^2]': AeffCol})
            a.write(f'{FakeAeffDir}/IC{season}_effectiveArea.csv',
...
def _clean_fake_data():
    ...
    FakeEventDir = config['FakeEventDir']
    FakeAeffDir = config['FakeAeffDir']
    shutil.rmtree(FakeEventDir)
    shutil.rmtree(FakeAeffDir)
...
def test_get_signal_map():
    ...
    assert np.round(np.max(maps[0]), 5) == 5.19153
    assert np.round(np.max(maps[1]), 5) == 5.53781
    assert np.round(np.max(maps[2]), 4) == 59.3064
    assert np.round(np.max(maps[3]), 2) == 1112.74
